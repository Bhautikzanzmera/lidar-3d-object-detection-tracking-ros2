...
class DAIRSyncedTrackingPublisher(Node):
    def __init__(self):
        super().__init__("dair_synced_tracking_publisher")
        ...
        self.declare_parameter("max_track_distance", 5.0)
        ...
        # Lightweight DAIR tracking memory
        self.tracks = {}
        ...
        self.get_logger().info("Topics: /points_raw, /dair_tracking_boxes, /camera/image_raw, /camera/dair_overlay")
    ...
    def assign_single_track(self, cls, center):
        center = np.asarray(center, dtype=np.float32)

        best_id = None
        best_dist = 1e9

        for track_id, trk in self.tracks.items():
            ...
            dist = float(np.linalg.norm(center[:2] - trk["center"][:2]))

            if dist < best_dist:
                best_dist = dist
                best_id = track_id

        if best_id is not None and best_dist <= self.max_track_distance:
            track_id = best_id
        else:
    ...
    def make_text_marker(self, obj, corners, marker_id, header):
        ...
        if track_id >= 0:
            marker.text = f"ID:{track_id} {obj['cls']} {obj['score']:.2f}"
        else:
    ...
    def publish_frame(self):
        ...
        if calib is not None:
            ...
            for i, obj in enumerate(labels):
                ...
                track_id = self.assign_single_track(obj["cls"], corners.mean(axis=0))
                obj["track_id"] = track_id

                marker_array.markers.append(self.make_box_marker(obj, corners, track_id, header))
                marker_array.markers.append(self.make_text_marker(obj, corners, 10000 + track_id, header))
                lidar_boxes += 1

        self.marker_pub.publish(marker_array)

        self.get_logger().info(
            f"Frame {frame_id}: points={len(pts)}, image_boxes={len(labels)}, tracked_boxes={lidar_boxes}, active_tracks={len(self.tracks)}",
...
def main(args=None):
    ...
    node = DAIRSyncedTrackingPublisher()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
